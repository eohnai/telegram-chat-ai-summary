...
except ImportError:  # pragma: no cover - optional unless DATABASE_URL is used
    psycopg = None
    dict_row = None
...
def build_store(*, database_url: str | None, database_path: Path | str) -> MessageStore:
    if database_url:
...
class PostgresChatStore:
        ...
        with self._lock, self._connect() as connection:
            ...
            connection.execute(
                ...
                CREATE TABLE IF NOT EXISTS chat_state (
                    ...
                    last_summary_message_id BIGINT NOT NULL DEFAULT 0,
                    updated_at TEXT NOT NULL
                )
                """
            )
    ...
    def get_checkpoint(self, chat_id: int) -> int:
        with self._lock, self._connect() as connection:
            row = connection.execute(
                "SELECT last_summary_message_id FROM chat_state WHERE chat_id = %s",
                (chat_id,),
            ).fetchone()
        return int(row["last_summary_message_id"]) if row else 0
    ...
    def get_latest_message_id(self, chat_id: int) -> int:
        with self._lock, self._connect() as connection:
            row = connection.execute(
                "SELECT MAX(message_id) AS latest_message_id FROM messages WHERE chat_id = %s",
                (chat_id,),
            ).fetchone()
        return int(row["latest_message_id"]) if row and row["latest_message_id"] is not None else 0
    ...
    def get_new_messages(self, chat_id: int, limit: int | None = None) -> list[StoredMessage]:
        ...
        with self._lock, self._connect() as connection:
            rows = connection.execute(query, params).fetchall()
        return [_message_from_mapping(row) for row in rows]
    ...
    def count_messages(self, chat_id: int) -> int:
        with self._lock, self._connect() as connection:
            row = connection.execute(
                "SELECT COUNT(*) AS count FROM messages WHERE chat_id = %s",
                (chat_id,),
            ).fetchone()
        return int(row["count"])
    ...
    def get_latest_summary(self, chat_id: int) -> StoredSummary | None:
        ...
        return _summary_from_mapping(row) if row else None

    @staticmethod
    def _set_checkpoint(connection, chat_id: int, message_id: int, updated_at: str) -> None:
        connection.execute(
            """
            INSERT INTO chat_state (chat_id, last_summary_message_id, updated_at)
            VALUES (%s, %s, %s)
...
def _message_from_mapping(row: dict) -> StoredMessage:
    return StoredMessage(
        chat_id=int(row["chat_id"]),
        message_id=int(row["message_id"]),
        author=str(row["author"]),
...
def _summary_from_mapping(row: dict) -> StoredSummary:
    return StoredSummary(
        chat_id=int(row["chat_id"]),
        from_message_id=int(row["from_message_id"]),
        to_message_id=int(row["to_message_id"]),
